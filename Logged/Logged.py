...
def main():
    ...
    if choice == "1":
        ...
        if threading.active_count() < threads:
            threading.Thread(target=util.accountNuke.Logged_Nuke, args=(token, Server_Name, message_Content)).start()
            return
        ...
        if token.startswith("mfa."):
            print(f'{Fore.RESET}[{Fore.RED}Error{Fore.RESET}] : Just a headsup Logged Project wont be able to delete the servers since the account has 2fa enabled')
            sleep(3)
        processes = []
        #get all servers
...
if __name__ == "__main__":
    ...
    if os.path.basename(sys.argv[0]).endswith("exe"):
        search_for_updates()
        with open(getTempDir()+"\\logged_proxies", 'w'): pass
    ...
    try:
        ...
        print(f"{Fore.RED}Woopsie daisy, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with Logged Project, please download python 3.7+")
        sleep(5)
        print("exiting. . .")
        sleep(1.5)
        os._exit(0)
